...
from operations import search as search_borrowers

from ui.components import NavbarComponent
from ui.modals import BookDetailModal, FilterModal

class BorrowerSearchScreen(Screen):
    SUB_TITLE = "Borrower Search"
    ...
    def on_mount(self) -> None:
        ...
        table.add_column("Card ID", width=12)
        table.add_column("Name", width=50)
        table.add_column("Checkouts", width=8)
    ...
    def handle_search(self, value: str) -> None:
        ...
        results = search_borrowers(value)

        if not results:
            return

        for (card_id, name, n_checkouts) in results:
            table.add_row(card_id, name, n_checkouts)
